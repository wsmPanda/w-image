...
export default {
                ...
                if (row && row.path) {
                  ...
                  return (
                    ...
                    >
                      <Icon
                        class="icon-arrow"
                        type={row.open === false ? "ios-arrow-forward" : "ios-arrow-up"}
                        onClick={() => this.onDictoryOpen(row, this.startIndex + index)}
                      ></Icon>
                      {this.$main.showCheck && (
                        <Icon
                          onClick={() => this.onCheck({ data: row.path })}
                          class={{
                      ...
                      <a onClick={() => this.onDictoryClick(row)}>{this.dictoryName(row)}</a>
                    </div>
                  )
                } else {
                  return [
                    <Thumbnail
                      ...
                      onCheck={(e) => this.onCheck({ data: row, e })}
                      onDblclick={() => this.onDbClick(row)}
  ...
  computed: {
    ...
    imageHeight() {
      return Number(this.imageSetting.height) + Number(this.imageSetting.margin)
    }
  },
  watch: {
    imageSetting: {
  ...
  methods: {
    ...
    closeDictory(index) {
      ...
      // 高度变化值
      let dHeight = this.heightList[index + row.length] - this.heightList[index]
    ...
    openDictory(index) {
      ...
      // 高度变化值
      let heightList = this.getHeightList(row.sub).list.map((item) => item + this.heightList[index])
    ...
    async updateList() {
      ...
      this.$forceUpdate()
      setTimeout(() => {
        if (this.scrollTarget && this.scrollTarget <= this.listHeight && this.$refs.listWrapper) {
    ...
    async pageChange() {
      ...
      let topHeight = Math.max(this.scrollTop - this.height * this.preloadPage, 0)
      if (topHeight <= 0) {
      ...
      while (
        this.startIndex > 0 &&
        this.heightList[this.startIndex - 1] === this.heightList[this.startIndex]
      ) {
    ...
    async checkPosition() {
      ...
      if (
        Math.abs(Math.max(0, this.transformTop - this.height * this.preloadPage) - this.scrollTop) >
        this.height
      ) {
        await this.pageChange()
        this.$forceUpdate()
    ...
    activeStepUp(v) {
      let index = this.data.findIndex((item) => this.activeImage === item)
      if (typeof this.data[index + Number(this.imageSetting.column) * v] === "string") {
